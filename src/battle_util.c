    ...
    case ABILITYEFFECT_MOVES_BLOCK: // 2
        {
            ...
            else if(((IsBattlerAlive(battler) && (gLastUsedAbility == ABILITY_DAZZLING || gLastUsedAbility == ABILITY_QUEENLY_MAJESTY || gLastUsedAbility == ABILITY_ARMOR_TAIL))||(IsBattlerAlive(partnerBattler) && (partnerAbility == ABILITY_DAZZLING || partnerAbility == ABILITY_QUEENLY_MAJESTY || partnerAbility == ABILITY_ARMOR_TAIL))) && (GetChosenMovePriority(gBattlerAttacker) > 0)&&(GetBattlerSide(gBattlerAttacker) != GetBattlerSide(battler)))
			{
                if (gBattleMons[gBattlerAttacker].status2 & STATUS2_MULTIPLETURNS)
                    gHitMarker |= HITMARKER_NO_PPDEDUCT;
